...
@bibliotecaPlanta_routes.route('/bibliotecaPlanta', methods=['GET','POST'])
@login_required
def bibliotecaPlanta():
    session['bibliotecaPlanta'] = True
    ...
    return render_template("bibliotecaPlantaRCA.html", data=data, usuario_actual=usuario_actual,titulo=titulo)

@bibliotecaPlanta_routes.route("/", methods=["GET", "POST"])
@login_required
def read_data_usuario():
    usuario_actual = current_user.username
    ...
    try:
        ...
        df_show = df[(df["Planta"] == planta_actual) & (df["Aprobado"] == True) & (df["Terminado"] == True)][columnas_existentes]
...
@bibliotecaPlanta_routes.route('/select/<int:index>')
@login_required
def select(index):
    initialize_data()
    ...
    if not cargarDatoID("evento", "ID", index):
        ...
        return redirect(url_for("bibliotecaPlanta.bibliotecaPlanta"))
...
@bibliotecaPlanta_routes.route('/reporte/<int:index>')
@login_required
def reporte(index):
    initialize_data()
...
@bibliotecaPlanta_routes.route('/delete', methods=['POST'])
@login_required
def delete():
...
@bibliotecaPlanta_routes.route('/download_report')
def download_report():
    # Example data
    data = session
